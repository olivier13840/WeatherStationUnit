...
// ======== CONFIG INFLUXDB ========
#define INFLUXDB_URL "http://10.182.40.139:8086"  //adresse IP du  serveur
#define INFLUXDB_TOKEN "yitOF8AflhJl1VrXGkKKXuH9hVHJe-RwFFFyD7GbYf6vVZlkBrtmpqIFSAKHlumfXCctA1HTLYLZIKgbEHh8Fg=="  //cle securite
#define INFLUXDB_ORG "ESIM"
#define INFLUXDB_BUCKET "Meteo_Station"

InfluxDBClient client(INFLUXDB_URL, INFLUXDB_ORG, INFLUXDB_BUCKET, INFLUXDB_TOKEN); 
Point weather("weather");


const unsigned long dureeAcquisition = 1UL * 60UL * 1000UL; // 1 min en ms
int lastMinute = -1;
...
void setup() {
  ...
  //verif connection InfluxDB
  if (client.validateConnection()) {
    Serial.println("Connexion à InfluxDB réussie !");
  } else {
    Serial.print("Erreur de connexion InfluxDB: ");
...
void loop() {
  ...
  // if ((minute == 38 || minute == 41 || minute== 44) && lastMinute != minute) {
...
void envoyerDonnees() {
  weather.addField("Temp", meanT);
  weather.addField("RH", meanRH);
  weather.addField("pm1",mc1p0);
  weather.addField("pm25", mc2p5);
  weather.addField("pm4",mc4p0);
  weather.addField("pm10", mc10p0);
